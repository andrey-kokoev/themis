...
import type { EnvelopeVerdict, EnvelopeViolation, PipelineStage } from "../types/envelope.js";
 ...
 */
const PIPELINE_RULES: Record<PipelineStage, { reads: string[]; writes: string[] }> = {
 ...
 */
function classifyPath(path: string): { authority: string; rule: string } | undefined {
...
export function validateRepo(
  operations: Array<{ type: "read" | "write"; path: string; stage?: PipelineStage }>
): EnvelopeVerdict {
      ...
      if (isUnder(op.path, "/policy/src") || isUnder(op.path, "/policy/spec")) {
        violations.push(
          createViolation(
            ...
            `Write to authoritative source path: ${op.path}`,
            op.path,
            { stage: op.stage, path: op.path }
          )
        );
      }
      ...
      if (op.stage) {
        ...
        if (!isAllowed && rules.writes.length > 0) {
          violations.push(
            createViolation(
              "IllegalStageWrite",
              `Stage "${op.stage}" may not write to ${op.path}`,
              op.path,
              { stage: op.stage, path: op.path, allowedWrites }
            )
          );
        }
      }
      ...
      if (isUnder(op.path, "/policy/state")) {
        ...
        if (op.stage && op.stage !== "journal") {
          violations.push(
            createViolation(
              "StateUsedAsSource",
              `Stage "${op.stage}" illegally reads from state: ${op.path}`,
              op.path,
              { stage: op.stage, path: op.path }
            )
          );
 ...
 */
export function validateStageOperation(
  stage: PipelineStage,
  operations: Array<{ type: "read" | "write"; path: string }>
): EnvelopeVerdict {
  ...
  for (const op of operations) {
    if (op.type === "read") {
      ...
      const isAllowed = 
        allowedReads.length > 0 && // Empty means no file reads (in-memory only)
      ...
      if (!isAllowed) {
        violations.push(
          createViolation(
            "IllegalStageRead",
            `Stage "${stage}" may not read from ${op.path}`,
            op.path,
            { stage, path: op.path, allowedReads }
          )
        );
      }
    }
    ...
    if (op.type === "write") {
      ...
      const allowedWrites = rules.writes;
      const isAllowed =
        allowedWrites.length === 0 || // Empty means in-memory only
      ...
      if (!isAllowed) {
        violations.push(
          createViolation(
            "IllegalStageWrite",
            `Stage "${stage}" may not write to ${op.path}`,
            op.path,
            { stage, path: op.path, allowedWrites }
          )
        );
      }
      ...
      if (isUnder(op.path, "/policy/src") || isUnder(op.path, "/policy/spec")) {
        violations.push(
          createViolation(
            "IllegalWriteToSource",
            `Stage "${stage}" attempted write to source: ${op.path}`,
            op.path,
            { stage, path: op.path }
          )
        );
 ...
 */
export function getDirectoryAuthority(path: string): { authority: string; rule: string } | undefined {
 ...
 */
export function isAuthoritativeSource(path: string): boolean {
 ...
 */
export function isRuntimeState(path: string): boolean {
  const classification = classifyPath(path);
 ...
 */
export function isCache(path: string): boolean {
  const classification = classifyPath(path);
