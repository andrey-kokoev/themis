...
class Parser {
  ...
  // Parse module file
  parseModule(): SurfaceModule {
    const imports: ImportDecl[] = [];
    const namespaces: NamespaceDecl[] = [];
    const aliases: RoleAliasDecl[] = [];
    const sharedIdentities: SharedIdentityDecl[] = [];
    ...
    while (!this.match("EOF")) {
      if (this.match("KEYWORD", "module")) {
        if (moduleId !== null) {
          throw new MultipleModuleRootsError(
            this.peek().line,
            this.peek().column
          );
        }
        ...
        while (!this.match("RBRACE") && !this.match("EOF")) {
          if (this.match("KEYWORD", "import")) {
            ...
            throw new ModuleParseError(
              `Unexpected token in module: ${this.peek().value}`,
              this.peek().line,
              this.peek().column
            );
          }
        ...
        throw new ModuleParseError(
          `Expected module or workspace, got: ${this.peek().value}`,
          this.peek().line,
          this.peek().column
        );
      }
  ...
  private parseAlias(): RoleAliasDecl {
  ...
  private parseSharedIdentity(): SharedIdentityDecl {
  ...
  private parseWorkspace(): Workspace {
    ...
    for (const keyword of compositionKeywords) {
      ...
      while ((match = regex.exec(workspaceBody)) !== null) {
        ...
        if (quoteCount % 2 === 0) {
          ...
          throw new InvalidPlacementError(
            `Composition declaration "${keyword}"`,
            startToken.line,
            startToken.column
          );
        }
...
// Public API
export function parseModule(input: string): SurfaceModule {
  const parser = new Parser(input);
  return parser.parseModule();
}

// Lowering function: SurfaceModule -> LoweredModule
export function lowerModule(surface: SurfaceModule): LoweredModule {
  return {
    tag: "LoweredModule",
    module: {
