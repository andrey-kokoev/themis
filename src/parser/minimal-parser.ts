...
class Parser {
  ...
  private checkEOF(context: string): void {
  ...
  private parseWorkspaceItems(): Workspace["items"] {
  ...
  private parseContextBlock(): ContextBlock {
  ...
  private parsePersistenceClause(): PersistenceClause {
    this.expectKeyword("persistence");
    const mode = this.expectString();
    return { tag: "PersistenceClause", mode };
  }

  private parseEquivalenceClause(): EquivalenceClause {
    this.expectKeyword("equivalence");
    const name = this.expectString();
    return { tag: "EquivalenceClause", name };
  }

  private parseRoleBlock(): RoleBlock {
    this.expectKeyword("role");
    const roleId = this.expectString();
    this.expectLBrace();

    let kind: string | undefined;
    let subject: SubjectBlock | undefined;
    const realizers: RealizerBlock[] = [];
    const witnesses: WitnessBlock[] = [];
  ...
  private parseSubjectBlock(): SubjectBlock {
  ...
  private parseRealizerBlock(): RealizerBlock {
  ...
  private parseWitnessBlock(): WitnessBlock {
  ...
  private parseRelationBlock(): RelationBlock {
...
export function parse(input: string): Workspace {
