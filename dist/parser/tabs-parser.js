...
class Parser {
    ...
    expect(type, value) {
        ...
        if (tok.type !== type || (value !== undefined && tok.value !== value)) {
            throw new Error(`Expected ${type}${value ? ` "${value}"` : ""} but got ${tok.type}${tok.value ? ` "${tok.value}"` : ""} at line ${tok.line}`);
        }
        return this.advance();
    }
    match(type, value) {
 ...
 */
export function parseTabsModule(input) {
    const parser = new Parser(input);
    return parser.parseModule();
}
//# sourceMappingURL=tabs-parser.js.map