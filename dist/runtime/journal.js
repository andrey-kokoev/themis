 ...
 */
export function append(journal, input) {
 ...
 */
export function appendOperatorAction(journal, action, verdict) {
    // J2.1: Strictly increasing sequence
    const seq = journal.nextSeq;
    // J2.2: Logical time
    const at = seq;
 ...
 */
export function replay(journal, workspaceResolver, stateResolver) {
    const mismatches = [];
...
function verdictsEqual(a, b) {
    if (a.admissible !== b.admissible)
        return false;
    if (a.satisfied.length !== b.satisfied.length)
        return false;
    if (a.unsatisfied.length !== b.unsatisfied.length)
        return false;
    ...
    for (let i = 0; i < aUnsatisfied.length; i++) {
        if (aUnsatisfied[i] !== bUnsatisfied[i])
            return false;
    }
    // Check conflicts
    ...
    for (let i = 0; i < aConflicts.length; i++) {
        if (aConflicts[i].type !== bConflicts[i].type)
            return false;
        if (aConflicts[i].message !== bConflicts[i].message)
            return false;
    }
    return true;
}
 ...
 */
export function getOperatorActions(journal) {
    return journal.records.filter((r) => r.tag === "OperatorAction");
}
