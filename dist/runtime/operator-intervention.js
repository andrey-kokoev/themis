 ...
 */
export function evaluateOperatorAction(state, action) {
    switch (action.tag) {
 ...
 * Reversible: no
 */
function evaluateRetryIntegration(state, action) {
 ...
 * Reversible: yes
 */
function evaluateAcknowledgeConflict(state, action) {
    if (!state.workspaces.includes(action.workspaceId)) {
        return {
            tag: "Inadmissible",
            reason: `Workspace "${action.workspaceId}" does not exist`,
 ...
 * Reversible: yes
 */
function evaluateRequestRebind(state, action) {
 ...
 * Reversible: yes
 */
function evaluateRecordNote(state, action) {
    const targetExists = action.scope === "workspace"
 ...
 */
export function canReplayAction(state, action) {
...
//# sourceMappingURL=operator-intervention.js.map