...
function createConflict(type, message, moduleIds, details) {
    return { type, message, moduleIds, details };
}
/**
 * Compute lifted role id according to E2.2 lifting rule.
 */
function computeLiftedRoleId(moduleId, localRoleId, namespace, aliases) {
    // E2.2 rule 1: Check for exact alias
    const alias = aliases.find(a => a.moduleId === moduleId && a.localRoleId === localRoleId);
    if (alias) {
 ...
 */
export function composeExplicit(modules, policy) {
    const conflicts = [];
    const notes = [];
    const liftingMap = {};
    // E1.1: Build namespace map from explicit declarations
    const namespaceMap = new Map(); // moduleId -> namespace
    for (const ns of policy.namespaces || []) {
        namespaceMap.set(ns.moduleId, ns.namespace);
    }
    // E1.2: Check namespace uniqueness
    const seenNamespaces = new Map(); // namespace -> moduleId
    for (const [moduleId, namespace] of namespaceMap) {
        if (seenNamespaces.has(namespace)) {
            conflicts.push(createConflict("NamespaceCollision", `Namespace "${namespace}" declared by both module "${seenNamespaces.get(namespace)}" and "${moduleId}"`, [seenNamespaces.get(namespace), moduleId], { namespace }));
        }
        seenNamespaces.set(namespace, moduleId);
    }
    // Sort modules for determinism
    const sortedModules = [...modules].sort((a, b) => a.moduleId.localeCompare(b.moduleId));
    ...
    const persistenceModes = new Set();
    const equivalenceModes = new Set();
    for (const mod of sortedModules) {
        const persistence = mod.workspace.items.find(i => i.tag === "PersistenceClause");
        const equivalence = mod.workspace.items.find(i => i.tag === "EquivalenceClause");
        if (persistence)
    ...
    if (persistenceModes.size > 1) {
        conflicts.push(createConflict("PersistenceModeConflict", `Different persistence modes: ${Array.from(persistenceModes).join(", ")}`, sortedModules.map(m => m.moduleId), { modes: Array.from(persistenceModes) }));
    }
    if (equivalenceModes.size > 1) {
        conflicts.push(createConflict("EquivalenceConflict", `Different equivalence modes: ${Array.from(equivalenceModes).join(", ")}`, sortedModules.map(m => m.moduleId), { modes: Array.from(equivalenceModes) }));
    }
    // E2.2: Compute lifted role ids
    const liftedRoles = [];
    for (const mod of sortedModules) {
        const namespace = namespaceMap.get(mod.moduleId);
        const roles = mod.workspace.items.filter(i => i.tag === "RoleBlock");
        for (const role of roles) {
            const liftedId = computeLiftedRoleId(mod.moduleId, role.roleId, namespace, policy.aliases || []);
            liftedRoles.push({
    ...
    for (const [liftedId, sources] of liftedIdSources) {
        ...
        if (sourceModules.size > 1) {
            conflicts.push(createConflict("AliasCollision", `Lifted role id "${liftedId}" comes from multiple sources`, Array.from(sourceModules), { liftedId, sources }));
        }
    }
    // E3: Check shared subject identity
    const subjectIdModules = new Map();
    for (const lifted of liftedRoles) {
    ...
    for (const [subjectId, moduleSet] of subjectIdModules) {
        if (moduleSet.size > 1) {
            ...
            const declaration = policy.sharedIdentities?.find(d => d.subjectId === subjectId);
            if (!declaration) {
                conflicts.push(createConflict("UndeclaredSharedIdentity", `Subject identity "${subjectId}" appears in multiple modules without explicit declaration`, Array.from(moduleSet), { subjectId, modules: Array.from(moduleSet) }));
            }
            else {
                // Check exact module set match
                const declaredModules = new Set(declaration.modules);
                const actualModules = moduleSet;
                const matches = declaredModules.size === actualModules.size &&
                    Array.from(declaredModules).every(m => actualModules.has(m));
                if (!matches) {
                    conflicts.push(createConflict("InvalidSharedIdentityDecl", `Shared identity declaration for "${subjectId}" does not match actual modules`, Array.from(moduleSet), {
                        subjectId,
    ...
    for (const mod of sortedModules) {
        const namespace = namespaceMap.get(mod.moduleId);
        const relations = mod.workspace.items.filter(i => i.tag === "RelationBlock");
        for (const rel of relations) {
            // Compute lifted endpoint ids
            const liftedSource = computeLiftedRoleId(mod.moduleId, rel.source, namespace, policy.aliases || []);
            const liftedTarget = computeLiftedRoleId(mod.moduleId, rel.target, namespace, policy.aliases || []);
            ...
            const sourceCandidates = liftedRoles.filter(l => l.liftedId === liftedSource);
            const targetCandidates = liftedRoles.filter(l => l.liftedId === liftedTarget);
            if (!validLiftedIds.has(liftedSource)) {
                conflicts.push(createConflict("RelationEndpointAmbiguousAfterAliasing", `Relation source "${rel.source}" (lifted: "${liftedSource}") does not resolve`, [mod.moduleId], { relation: rel, endpoint: "source", liftedId: liftedSource }));
            }
            if (!validLiftedIds.has(liftedTarget)) {
                conflicts.push(createConflict("RelationEndpointAmbiguousAfterAliasing", `Relation target "${rel.target}" (lifted: "${liftedTarget}") does not resolve`, [mod.moduleId], { relation: rel, endpoint: "target", liftedId: liftedTarget }));
            }
        }
    }
    ...
    const seenContextKeys = new Map(); // key -> value
    for (const mod of sortedModules) {
        const contexts = mod.workspace.items.filter(i => i.tag === "ContextBlock");
    ...
    const composedRoles = liftedRoles.map(lr => ({
        ...lr.role,
        roleId: lr.liftedId,
    }));
...
//# sourceMappingURL=explicit-composition.js.map