...
export function compose(modules) {
    ...
    for (const mod of sortedModules) {
        if (workspaceIds.has(mod.workspace.name)) {
            conflicts.push(createConflict("WorkspaceIdCollision", `Workspace id "${mod.workspace.name}" appears in multiple modules`, sortedModules.filter(m => m.workspace.name === mod.workspace.name).map(m => m.moduleId)));
        }
    ...
    for (const [roleId, moduleIdList] of roleIds) {
        if (moduleIdList.length > 1) {
            conflicts.push(createConflict("RoleIdCollision", `Role id "${roleId}" appears in multiple modules`, moduleIdList, { roleId }));
        }
    }
    ...
    for (const [subjectId, moduleIdList] of subjectIds) {
        if (moduleIdList.length > 1) {
            conflicts.push(createConflict("SubjectCollision", `Subject identity "${subjectId}" appears in multiple modules`, moduleIdList, { subjectId }));
        }
    }
    // C2.4: Check context key value collisions
    const contextKeys = new Map(); // key -> value -> moduleIds
    for (const mod of sortedModules) {
    ...
    for (const [key, valueMap] of contextKeys) {
        if (valueMap.size > 1) {
            ...
            conflicts.push(createConflict("ContextKeyCollision", `Context key "${key}" has different values in different modules`, allModules, { key, values: Array.from(valueMap.keys()) }));
        }
    }
    ...
    for (const mod of sortedModules) {
        ...
        for (const rel of relations) {
            if (!validRoleIds.has(rel.source)) {
                conflicts.push(createConflict("RelationEndpointMissing", `Relation source "${rel.source}" does not resolve to any role`, [mod.moduleId], { relation: rel, endpoint: "source", endpointId: rel.source }));
            }
            if (!validRoleIds.has(rel.target)) {
                conflicts.push(createConflict("RelationEndpointMissing", `Relation target "${rel.target}" does not resolve to any role`, [mod.moduleId], { relation: rel, endpoint: "target", endpointId: rel.target }));
            }
        }
    }
...
//# sourceMappingURL=conservative-composition.js.map