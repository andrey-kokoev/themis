 ...
 */
export function wellFormed(workspace) {
    const errors = [];
    // Check workspace id
 ...
 */
export function normalize(workspace) {
    // Sort roles by id
    const sortedRoles = [...workspace.roles].sort((a, b) => a.id.localeCompare(b.id));
    ...
    const sortedContext = new Map([...workspace.context.entries()].sort(([a], [b]) => a.localeCompare(b)));
    return {
        ...workspace,
        roles: sortedRoles,
...
export function equiv(a, b) {
    ...
    // Compare ids
    if (normA.id !== normB.id)
        return false;
    // Compare context
    if (normA.context.size !== normB.context.size)
        return false;
    ...
    if (normA.persistence !== normB.persistence)
        return false;
    if (normA.equivalence !== normB.equivalence)
        return false;
    // Compare roles
    if (normA.roles.length !== normB.roles.length)
        return false;
    for (let i = 0; i < normA.roles.length; i++) {
        if (!rolesEqual(normA.roles[i], normB.roles[i]))
            return false;
    }
    // Compare relations
    if (normA.relations.length !== normB.relations.length)
        return false;
    for (let i = 0; i < normA.relations.length; i++) {
        if (!relationsEqual(normA.relations[i], normB.relations[i]))
            return false;
    }
    return true;
}
function rolesEqual(a, b) {
    ...
    if (a.kind !== b.kind)
        return false;
    if (!subjectsEqual(a.subject, b.subject))
        return false;
    if (a.realizers.length !== b.realizers.length)
        return false;
    if (a.witnesses.length !== b.witnesses.length)
        return false;
    return true;
}
 ...
 */
export function satisfiedRole(workspace, facts, roleId) {
    ...
    const subjectObserved = facts.some(f => f.tag === "SubjectObserved" && f.subjectId === role.subject.identity);
    if (!subjectObserved) {
        reasons.push(`Subject "${role.subject.identity}" not observed`);
    }
    // Check realizer observed
    const realizerObserved = facts.some(f => f.tag === "RoleRealized" &&
        f.roleId === roleId &&
 ...
 */
export function satisfiedWorkspace(workspace, facts) {
    const reasons = [];
    ...
    const validRoleIds = new Set(workspace.roles.map(r => r.id));
    for (const relation of workspace.relations) {
        const observed = facts.some(f => f.tag === "RelationObserved" &&
            f.kind === relation.kind &&
            f.source === relation.source &&
            f.target === relation.target);
        if (!observed) {
            allSatisfied = false;
            reasons.push(`Relation "${relation.kind}" from "${relation.source}" to "${relation.target}" not observed`);
        }
    }
