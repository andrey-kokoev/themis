 ...
 */
export function buildTmuxPaneCommands(module) {
    const commands = [];
 ...
 */
function buildPipeMap(pipes) {
    const map = new Map();
    for (const pipe of pipes) {
        // from writes to to
        if (!map.has(pipe.from))
            map.set(pipe.from, {});
        if (!map.has(pipe.to))
            map.set(pipe.to, {});
...
function buildTabCommands(sessionName, tab, pipeMap) {
    const commands = [];
    commands.push({
        tag: "tmux",
...
function buildTabPanes(sessionName, tab, pipeMap, skipFirstWindow) {
    const commands = [];
 ...
 */
function buildStartupCommands(sessionName, startup, tabs) {
    const commands = [];
    // Build pane name -> window index map
    const paneWindowMap = new Map();
...
//# sourceMappingURL=tmux-pane-builder.js.map